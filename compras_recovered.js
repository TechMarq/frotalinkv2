...
async function renderIntegracao() {
    ...
    try {
        ...
        comprasParaIntegracao.forEach(comp => {
            const fornecedorNome = comp.fornecedores ? comp.fornecedores.nome : 'Sem Fornecedor';
            const formaNome = comp.formas_pagamento ? comp.formas_pagamento.nome : 'N/A';
        ...
        if (err.message && err.message.includes('integrado_financeiro')) {
            alert("⚠️ ALERTA DE BANCO DE DADOS ⚠️\n\nA coluna 'integrado_financeiro' não existe na tabela compras.\nVocê precisa rodar o script '10_compras_integracao.sql' no Supabase para ativar esta funcionalidade!");
            tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 2rem; color: #ef4444; font-weight: bold;">⚠️ Execute o script 10_compras_integracao.sql no banco de dados para habilitar!</td></tr>';
        } else {
            tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 2rem; color: #ef4444;">Erro ao carregar dados.</td></tr>';
        }
    }
}

window.toggleSelectAllIntegracao = () => {
    const isChecked = document.getElementById('selectAllIntegracao').checked;
...
window.integrarAoFinanceiro = async () => {
    ...
    try {
        ...
        for (const id of ids) {
            ...
            let lancamentos = [];
            const fornecedorNome = comp.fornecedores ? comp.fornecedores.nome : 'Fornecedor não especificado';
            ...
            if (comp.financeiro_parcelado) {
                ...
                if (parcelas && parcelas.length > 0) {
                    for (const p of parcelas) {
                        lancamentos.push({
                            tipo: 'PAGAR',
                            descricao: `Referente NF ${comp.numero_nota || 'S/N'} (Parc ${p.numero_parcela}/${comp.qtd_parcelas}) - ${fornecedorNome}`,
                            ...
                            compra_id: comp.id,
                            forma_pagamento_id: comp.forma_pagamento_id,
